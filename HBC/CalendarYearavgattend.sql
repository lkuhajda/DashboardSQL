...
(select  count (distinct(MinistryWeekEndLabel))  FROM [Analytics].[DW].[DimDate] t2 
		where td.CalendarMonth = t2.CalendarMonth and td.CalendarYear = t2.CalendarYear 
		and month(MinistryWeekEndLabel) = td.CalendarMonth )  as AveAtt
...
WHERE td.[CalendarYear] = 2016
--td.[CalendarYear] in (@ReportYear, (@ReportYear-1), (@ReportYear-2))
AND td.[CalendarMonth] = @ReportMonth
AND 
	--(
	--	(tm.[name] LIKE  '%Harvest Kids%') -- OR   (tm.[name] like  '%Harvest Kids%')
	--	OR (tv.[name] LIKE '%Worship%' )
	--)
	
	ISNULL(NULLIF(at.Category,''), 'Attendee') = 'Attendee'
    AND ( 
...
group by td.[CalendarYear], td.[CalendarMonth] 
