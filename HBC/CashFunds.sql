...
SELECT
	...
	CASE fundcode
		WHEN  '035' THEN 'Reserve'
		WHEN  '053' THEN 'Mortgage Sinking Fund'
		WHEN  '064' THEN 'Systems Project'
		WHEN  '067' THEN 'HCA Endowment'
		WHEN  '275' THEN 'Camp Reserve'
	END as FundName
	...
	(
		...
		OR
		(fc.FundCode = '275' AND GLCode IN ('10016','10025','20010','20020'))  Special - Camp (Camp Reserve)
	)
	GROUP BY fundcode
...
SELECT
	...
	SUM(t1.amount) as Total,  'General' as FundName
	--t1.amount, t1.DateID, t3.[ActualDate] 
	FROM [Analytics].[DW].[FactFinancialOther] t1
	INNER JOIN [Analytics].[DW].[DimFinancialCategory] fc
	ON t1.[FinancialCategoryID] = fc.[FinancialCategoryID]
